...
class Brain{
	...
	int grab_counter; // The value is for loop counter to watch the received data from the robot when grabbed the ball
	WirelessUnit xbee;
	...
	void updateState(Robot ourRobot){
		...
		if( this->state == 1){
			...
			if(msg == '1'){
				...
				cout<<"Didnt receive from the robot, grab_counter:"<<grab_counter<<"\n";
				if(grab_counter % 100 == 0){
					this->state = 1;
					grab_counter = 1;
				}
				grab_counter++;
			...
			if( (finalAngle>20 and finalAngle<340) or (finalAngle<-20 and finalAngle>-340) ){
				if(this->preparationCounter % PREPARATION_FREQUENCY == 0){
